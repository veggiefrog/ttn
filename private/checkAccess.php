...
// <rsp stat='ok' />
//
function va3ned_ttn_checkAccess(&$ciniki, $tnid, $method) {
    //
    ...
    $rc = ciniki_tenants_checkModuleAccess($ciniki, $tnid, 'va3ned', 'ttn');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    $modules = $rc['modules'];
    ...
    if( !isset($rc['ruleset']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'va3ned.ttn.1', 'msg'=>'No permissions granted'));
    }
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'va3ned.ttn.2', 'msg'=>'Access denied.'));
    }
    //
    ...
    // By default fail
    //
    return array('stat'=>'fail', 'err'=>array('code'=>'va3ned.ttn.3', 'msg'=>'Access denied'));
}
?>
